    ...
    configState_(config)
{
    ...
    existingRadio = new QRadioButton(tr("PWB time-lag cache available : "));
    existingRadio->setToolTip(tr("<b>PWB time-lag cache available:</b> Select a per-period PWB cache generated by a previous run. EddyFlow will reuse its timestamp- and gas-specific lags, which avoids rerunning PWB. Missing entries are calculated and saved to a new cache in the current output directory."));
    ...
    fileBrowse->setToolTip(tr("<b>Load:</b> Load an existing PWB per-period time-lag cache"));
    fileBrowse->setDialogTitle(tr("Select the PWB Time-Lag Cache File"));
...
void PwbTimelagSettingsDialog::testSelectedFile(const QString& fp)
{
    ...
    // PWB caches are per-period CSV files, not aggregate time-lag assessment
    // files. Their version and compatibility are validated by the engine.
