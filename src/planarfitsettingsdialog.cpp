    ...
    configState_(config)
{
    ...
    groupTitle->setText(tr("If you already ran EddyFlow with the Planar Fit option for this dataset, <br />"
                           "you probably have a file named \"EddyFlow_<i>projID</i>_planarfit.txt\" in the results folder. <br />"
    ...
    subsetCheckBox->setToolTip(tr("<b>Select a different period:</b> Select this option and set the corresponding dates to specify the time period EddyFlow will use for the planar fit calculations. This subperiod must fall within the time period defined by the available raw data, but does not necessarily need to fall within the subperiod that you may have possibly selected in the Basic Settings page."));
    ...
    maxAvgWLabel->setToolTip(tr("<b>Maximum mean vertical wind component:</b> Set a maximum vertical wind component to instruct EddyFlow to ignore flux averaging periods with larger mean vertical wind components, when calculating the rotation matrices. Using elements with too large (unrealistic) vertical wind components would corrupt the assessment of the fitting plane, and of the related rotation matrices."));
    ...
    minAvgULabel->setToolTip(tr("<b>Minimum mean horizontal wind component:</b> Set a minimum horizontal wind component to instruct EddyFlow to ignore flux averaging periods with smaller mean horizontal wind components, when calculating the rotation matrices. When the horizontal wind is very small, the attack angle may be affected by large errors, and so would be the vertical wind component, resulting in poor quality data that would degrade the planar fit assessment."));
    ...
    setEquallySpacedButton->setToolTip(tr("<b>Set equally spaced:</b> Clicking this button will cause the EddyFlow to divide the whole 360&deg; circle in n equally-wide sectors, when n is the number of sectors currently entered. Note that the operation will not modify the north offset."));
