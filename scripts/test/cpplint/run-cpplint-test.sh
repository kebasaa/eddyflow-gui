...
SCRIPT_DIR=$(pwd)
VERSION_TO_TEST=$1
SRC_DIR=~/devel/EddyFlow/gui/source/src
TEST_DIR=~/devel/EddyFlow/gui/test/cpplint
PREFIX='../../source/'
NEW_LIST_FILE="new-files-in-v$VERSION_TO_TEST.txt"
NEW_FILE_DIR="new-ep-v$VERSION_TO_TEST-files"
CPPLINT_EP="cpplint_EddyFlow.py"
...
usage() {
  echo
  echo "Get the list of EddyFlow files added from the previous published version (master branch),"
  echo "and run cpplint on each one. Version $SCRIPT_VERSION."
  echo
  echo "Usage: $0 <EddyFlow-version>"
  echo
  echo "<EddyFlow_version> Specify current version number in short form, for example '6.0.0'"
  echo
...
RESULT_FILE="cpplint-EddyFlow-v$VERSION_TO_TEST-results.txt"

# create folder with version name in the test dir
cd $TEST_DIR
mkdir $TEST_DIR_VERSION 2> /dev/null

# get the list of EddyFlow files added from the previous published version (master branch)
cd $SRC_DIR
